...
| multi-decree (epoch-keyed synod → splice reader) | learner facts + spliced log EC — **inferred** (learning echo cycle); per-slot chosen = irrefutable — untyped (sim) | TO (spliced log) | static | **✓ shown** (per-slot agreement ∀ under dueling leaders ±acceptor crash; learners converge at quiescence; no-adoption and sub-majority reds refuted, fuzz) | **✓ shown** (Ω discipline, fuzz) |
