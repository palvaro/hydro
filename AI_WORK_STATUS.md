...
`sandbox-64b2e9b3-375c-408a-89af-0583efa4b9f2`, commits `76e88cd06f`, `ba0def4008`
...
`76e88cd06f` + `ba0def4008` are the provenance handoff: a sim-time forward-lineage tool that
...
- `hydro_test/src/cluster/provenance_ground_truth.rs` — 7 tests: heartbeat, timeout/retry (+ loop
  gain), gossip (+ loop gain, observation only), transitive closure, Raft.
...
own dedup gates are the fallback. Several node kinds unsupported (panic with a clear message).
