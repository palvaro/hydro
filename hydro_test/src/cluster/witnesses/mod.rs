...
//! | `compaction_falls_behind` | `compaction_reserve = 0` | hazardous, by exhibited collapse; work modeled by a counter, excluded from the checker scorecard | gets pay per uncompacted record and starve compaction | tail 205 ops served against 1000, backlog 3432 -> 4223 | `background_compaction_starves_and_reads_collapse` |
//! | `compaction_falls_behind` | `compaction_reserve = 8` | no ground truth; work modeled by a counter, excluded from the checker scorecard | same coupling, reserve keeps the segment short | extra units 0 / 4 / 52 / 102 / 195 for holds of 0 / 4 / 8 / 16 / 32 rounds | `held_arrivals_cost_extra_units_growing_with_the_hold` |
...
//!
//! The `compaction_falls_behind` rows are labeled but excluded from the checker scorecard. The
//! program computes each read's cost as an integer inside a record and subtracts it from an
//! integer budget; it never moves the records it claims to scan, so no work actually increases
//! and a tool that counts records is right to see nothing. The rows return to the scorecard if
//! the witness is rewritten so that a read joins against the uncompacted segment and produces one
//! record per record scanned.
//!
