//! Plain retries with no governance.
//!
//! | baseline per round | capacity | trigger | timeout | asserted tail (rounds 600..800) | label and basis |
//! |---:|---:|---|---:|---|---|
//! | 2 requests | 5 serves | 12/round in rounds 100..160 | 6 ticks | 800 repeated sends, 666 repeated serves; backlog 2,119 -> 2,318 | hazardous, by exhibited collapse |
//!
//! The bounded trigger ends at round 160, but redundant sends and serves remain in rounds 600
//! through 800 and the server backlog is still growing. The test therefore exhibits a collapse.
//! Each request is sent on every timeout until it reaches three attempts. The cap limits the work
//! caused by one request, but continuing input supplies new requests whose retries sustain the
//! overloaded state. A deployment wires `client.source_interval(period)` and
//! `server.source_interval(period)` to the clock parameters of
//! [`retry_service`](super::retry_service).
...
mod sim_tests {
    ...
    const CONFIG: RetryConfig = RetryConfig { timeout_ticks: 6, server_capacity: 5, policy: Policy::None { max_attempts: 3 } };
    ...
    /// copies before retries. Each delayed request can produce two retries. If the disturbance
    /// pushes the continuing baseline into repeated timeout, two new requests can offer up to six
    /// copies against capacity five. The redundant load should then delay subsequent baseline replies and persist into the distant
    /// tail, even though every individual request stops after three attempts.
    ...
    fn bounded_disturbance_has_a_distant_tail() {
        ...
        assert_eq!(
            (tail.first().unwrap().backlog, tail.last().unwrap().backlog),
            (2119, 2318)
        );
    }
