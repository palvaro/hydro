//! A retry client governed by a hybrid token bucket.
//!
//! | baseline per round | capacity | trigger | timeout | asserted tail (rounds 600..800) | label and basis |
//! |---:|---:|---|---:|---|---|
//! | 2 requests | 5 serves | 12/round in rounds 100..160 | 6 ticks | 691 repeated sends, 634 repeated serves; backlog 914 -> 1,005 | hazardous, by exhibited collapse |
//!
...
//!
//! The client gains three tokens per client timer element and one per four useful replies, retains at most forty, and spends one
...
mod sim_tests {
    ...
    const CONFIG: RetryConfig = RetryConfig { timeout_ticks: 6, server_capacity: 5, policy: Policy::HybridBudget { initial_tokens: 0, capacity: 40, refill_per_tick: 3, replies_per_token: 4 } };
    ...
    /// copies before retries. At least three refill tokens per round then permit retries while two new
    /// requests continue, eventually offering more than capacity five. The queue should therefore keep
    /// growing after round 160, and the distant tail should contain hundreds of retries and at most
    ...
    fn bounded_disturbance_has_a_distant_tail() {
        ...
        assert_eq!(tail.iter().map(|r| r.repeated_sent).sum::<usize>(), 691);
        assert_eq!(tail.iter().map(|r| r.repeated_served).sum::<usize>(), 634);
        assert_eq!((tail.first().unwrap().backlog, tail.last().unwrap().backlog), (914, 1005));
    }
