...
/// is an O(n²) echo among learners (like broadcast-transcript, unlike
/// Raft's O(n)). Committed values are routed back by member 0 only, after
/// a `unique()` dedup — the wrapper's redo queue may legitimately place a
/// value at multiple slots, and the closed-loop client must see exactly one
...
pub fn multi_paxos_bench<'a>(
    ...
    // Election timer: only member 0 fires quickly, so it wins leadership at
    // startup and (since elections are stall-triggered) keeps it for the
    // whole run. Others effectively never campaign.
    ...
    let latencies = bench_client(
        ...
        |input| {
            ...
            // Route committed values back from member 0 only (every learner
            // converges to the same set; three responders would break the
            // closed loop), deduplicated (redo-queue duplicates).
            outs.learned
                .weaken_consistency()
