...
/// Echo server with Lamport-clock stamped messages.
#[derive(Parser, Debug)]
struct Opts {
...
pub const DEFAULT_SERVER_ADDRESS: &str = "localhost:11052";
...
fn test() {
    ...
    let (_server, _, mut server_stdout) =
        run_cargo_example("lamport_clock", "--role server --address 127.0.0.1:11052");

    let (_client1, mut client1_stdin, mut client1_stdout) =
        run_cargo_example("lamport_clock", "--role client --address 127.0.0.1:11052");
    ...
    wait_for_process_output(
        &mut server_output,
        &mut server_stdout,
        "Server is live! Listening on 127\\.0\\.0\\.1:11052\n",
    );
