...
fn test() {
    ...
    let mut server = run_current_example!("--role server --address 127.0.0.1:2061");
    server.wait_for_output("Server is live! Listening on 127.0.0.1:2061");

    let mut client1 = run_current_example!("--role client --address 127.0.0.1:2061");
    ...
    let mut client2 = run_current_example!("--role client --address 127.0.0.1:2061");
    ...
    client1.wait_for_output(r#"Got a Response: KvsResponse \{ key: "a", value: "8" \}"#);
}
