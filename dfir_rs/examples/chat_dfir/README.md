...
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --role server
```

#### Running the Clients
In another terminal run the first client:
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "alice" --role client
```

In the third terminal run the second client:
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "bob" --role client
```
...
##### First
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --address 127.0.0.1:12345  --role gossiping-server1 
```
##### Second
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --address 127.0.0.1:12346  --role gossiping-server2 
```
##### Third
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --address 127.0.0.1:12347  --role gossiping-server3 
```

##### Fourth
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --address 127.0.0.1:12348  --role gossiping-server4 
```

##### Fifth
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --address 127.0.0.1:12349  --role gossiping-server5 
```
#### Running the Clients
In another terminal run the first client:
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "alice" --address 127.0.0.1:12345 --role client
```

In another terminal run the second client:
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "bob" --address 127.0.0.1:12349 --role client
```
...
#### Client
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "alice" --role client --graph mermaid
```
#### Broadcast Server
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --role server --graph mermaid
```

#### Gossip Server
```shell
cargo run -p dfir_rs --example chat_dfir -- --name "_" --role gossiping-server1 --graph mermaid
```

### Display Help
```shell
cargo run -p dfir_rs --example chat_dfir -- --help
```