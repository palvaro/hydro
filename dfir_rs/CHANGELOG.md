...
<details>
<summary>Original prompt</summary>
This section details on the original issue you should resolve<issue_title>refactor(dfir_rs): Use std::task::Wake instead ofArcWake in context waker implementation</issue_title><issue_description>indfir_rs/src/scheduled/context.rs</issue_description>   <!-- START COPILOT ORIGINAL PROMPT -->
   <details>
   <summary>Original prompt</summary>
This section details on the original issue you should resolve<issue_title>refactor(dfir_rs): Use std::task::Wake instead ofArcWake in context waker implementation</issue_title><issue_description>indfir_rs/src/scheduled/context.rs</issue_description><csr-unknown/>
...
<csr-unknown>
Pull Request OverviewThis PR refactors the dfir_rs library by removing the demux operatorand replacing its usage with the demux_enum operator, which providesbetter type safety and ergonomics.Pull Request OverviewThis PR refactors the dfir_rs library by removing the demux operatorand replacing its usage with the demux_enum operator, which providesbetter type safety and ergonomics.Pull Request OverviewThis PR refactors the dfir_rs library by removing the demux operatorand replacing its usage with the demux_enum operator, which providesbetter type safety and ergonomics.<csr-unknown/>
