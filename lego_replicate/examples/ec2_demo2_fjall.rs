//! Interactive EC2 demo with fjall backend.
//!
//! f=2: 5 replicas + 1 router = 6 EC2 instances.
//! Commands: PUT <key> <value> | GET <key> | quit
//!
//! Run with:
//!   cargo run -p lego_replicate --features backend_fjall --example ec2_demo2
...
fn build_pipeline<'a>(
    ...
    let responses = committed_puts
        ...
        .all_ticks()
        .scan(
            q!(|| lego_replicate::applier::FjallApplierState::new()),
            q!(|state: &mut lego_replicate::applier::FjallApplierState,
...
async fn main() {
    ...
    println!("=== Lego Replicate EC2 Demo (fjall) ===");
