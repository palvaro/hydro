...
until we run it.
We can run this flow from within Rust via the [`run_available_sync()` method](rust:dfir_rs::scheduled::context::Dfir::run_available_sync).
...
we may need a different method than `run_available_sync()`, e.g. the [`run()`](rust:dfir_rs::scheduled::context::Dfir::run) method, check [the networking examples](./example_7_networking.mdx) for more.
