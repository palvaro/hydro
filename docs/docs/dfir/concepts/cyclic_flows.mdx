...
import exampleCode2 from '!!raw-loader!../../../../dfir_rs/examples/example_naturals.rs.disabled';
import { getLines, extractOutput, extractMermaid } from '../../../src/util';

# Dataflow Cycles and Fixpoints

:::warning
TODO(inline): The examples on this page use intra-tick cycles which are not yet supported in the inline DFIR codegen path. The example code is preserved but currently disabled.
:::
...
Like many looping constructs, it is possible to write a cyclic DFIR program that never ``terminates``, in the sense that it produces an unbounded stream of data. If we use `[join_multiset()](../syntax/surface_ops_gen.md#join_multiset)` instead of `[join()](../syntax/surface_ops_gen.md#join)` in our Reachability dataflow, the call to `flow.run_available()` never terminates, because each time the same vertex is visited, new data is generated!
...
`[inspect()](../syntax/surface_ops_gen.md#inspect)` operator along an edge of the cycle to see if it's generating unbounded amounts of duplicated data. You can use the `[unique()](../syntax/surface_ops_gen.md#unique)` operator to ameliorate the problem.