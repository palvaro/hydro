...
Hydro is a distributed dataflow language, powered by the high-performance single-threaded [DFIR runtime](../dfir/). Unlike traditional architectures such as actors or RPCs, Hydro offers _choreographic_ APIs, where expressions and functions can describe computation that takes place across many locations. It also integrates with [Hydro Deploy](deploy/) to make it easy to deploy and run distributed Hydro programs either locally or in the cloud.
