...
There are a [variety of APIs](pathname:///rustdoc/hydro_lang/live_collections/struct.Stream#impl-Stream<T,+Process<'a,+L>,+B,+O,+R>) for sending data to and receiving data from clusters. For example, we `broadcast_bincode` to send copies to all members (e.g. for replication), or use `send_bincode` if we have a custom partitioning algorithm.
...
Finally, we will send the data back to the leader. To identify each sender, `send_bincode` emits a keyed stream with `MemberId` keys. Here, we do not care about the sender, so we use `.values()` to interleave just the data.
